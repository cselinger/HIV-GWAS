...
#         is.na(pca7) | is.na(pca8) | is.na(pca9) | is.na(pca10) | is.na(id)))
          ...
          incomplete <- with(regData, (is.na(sqrtcd4_3uart) | is.na(sqrtcd4_0uart)| is.na(geno) |
          is.na(age_base) | is.na(sex_base) |  is.na(cohort)| is.na(pca1_base) | is.na(pca2_base) | is.na(pca3_base) | is.na(pca4_base) | 
          is.na(pca5_base) | is.na(pca6_base) | is.na(pca7_base) | is.na(pca8_base) | is.na(pca9_base) |is.na(pca10_base) ) )         
...
#         na.action=na.omit, data=regData[!incomplete,]);
          ...
          mod <- lm(sqrtcd4_3uart ~  sqrtcd4_0uart + geno  + sex_base + age_base 
...
#        pca10, na.action=na.omit, data=regData[!incomplete,] );
        ...
        basemod <- lm(sqrtcd4_3uart ~  sqrtcd4_0uart  + sex_base + age_base
                      + pca1_base + pca2_base + pca3_base + pca4_base + pca5_base 
                      + pca6_base + pca7_base + pca8_base + pca9_base + pca10_base,
                      na.action=na.omit, REML=F,
                      data=regData[!incomplete & regData$cohort=="UARTO",] );
